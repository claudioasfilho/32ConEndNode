  ...
  switch (SL_BT_MSG_ID(evt->header)) {
    ...
    case sl_bt_evt_system_boot_id:
      ...
      app_assert_status(sc);
           ...
           sprintf(devicename,"Silabs %02x:%02x:%02x:%02x:%02x:%02x",address.addr[5],address.addr[4],address.addr[3],address.addr[2],address.addr[1],address.addr[0]);
      ...
      //Power Set to 1 dBm
      sc = sl_bt_system_set_tx_power(10, 10, &min_power,&max_power);
    ...
    case sl_bt_evt_connection_opened_id:
     ...
     sc = sl_bt_connection_set_parameters( evt->data.evt_connection_opened.connection, 8, 800, 0, 2000, 0, 65535);
     app_assert_status(sc);
